...
description: >
  ...
  and misnamed entities. Delegates all writes to /yoke:preserve.
  ...
  "align vault", "fix backlinks", "fix misalignments", "/yoke:compress".
user_invocable: true
allowed-tools: Bash, Read, Glob, Grep, Skill, Agent
---

# /yoke:compress — Vault Alignment Engine
...
Check if the input arguments include `--memory <name>`. If found, extract the vault name and remove it from the arguments before parsing `--mode`.

**Step 2 — Resolve vault path:**

1. **If `--memory <name>` was provided:**
   Read the memory registry at `<plugin_dir>/memories.json`. Find the entry matching the name.
   If not found: error — "Vault `<name>` is not registered. Run `/yoke:memory` to see available vaults."
   If found: set `YOKE_MEMORY_PATH` to the entry's `path` value. Store the resolved vault name as `YOKE_MEMORY_NAME`.

2. **If no `--vault` flag — CWD detection:**
   Read `<plugin_dir>/memories.json`. Check if the current working directory is inside any registered vault path
   (CWD starts with a registered vault's absolute path). If multiple match, use the longest path (most specific).
   If found: set `YOKE_MEMORY_PATH` to the matching vault's `path`. Store its name as `YOKE_MEMORY_NAME`.

3. **If CWD detection fails — default vault:**
   From the registry, find the vault with `"default": true`.
   If found: set `YOKE_MEMORY_PATH` to the default vault's `path`. Store its name as `YOKE_MEMORY_NAME`.

4. **If no resolution:**
   Error — "No vault resolved. Available vaults:" followed by the registry listing.
   "Use `--memory <name>` to specify, or run `/yoke:bootstrap` to register a memory."

**Step 3 — Validate vault path:**
```bash
test -d "<YOKE_MEMORY_PATH>" && echo "exists" || echo "missing"
```
If missing: error — "Vault path `<YOKE_MEMORY_PATH>` does not exist on disk. Run `/yoke:bootstrap` to re-register."

**Step 4 — Read vault config:**
```bash
cat <YOKE_MEMORY_PATH>/.yoke-memory/config.json 2>/dev/null
```
Extract `language`, `git.strategy`, and other relevant fields for use in later phases.

**From this point forward, ALL vault file operations use `<YOKE_MEMORY_PATH>` as the root.**
- Entity directories: `<YOKE_MEMORY_PATH>/actors/`, `<YOKE_MEMORY_PATH>/people/`, etc.
- Git operations: `git -C <YOKE_MEMORY_PATH> <command>`
- When delegating to `/yoke:preserve`, pass `--memory $YOKE_MEMORY_NAME`
...
proposes fixes to the user, and delegates all writes to `/yoke:preserve`.
...
**Critical rules:**
- **NEVER** write entity files directly — all mutations go through `/yoke:preserve`
...
Execute:
```bash
git -C <YOKE_MEMORY_PATH> pull --rebase origin main
```

If it fails:
- No remote: warn "No remote configured. Working locally." and proceed.
- Conflict: `git -C <YOKE_MEMORY_PATH> rebase --abort` and warn the user. Do NOT proceed without resolving.
...
For each entity directory (`<YOKE_MEMORY_PATH>/actors/`, `<YOKE_MEMORY_PATH>/people/`, `<YOKE_MEMORY_PATH>/teams/`, `<YOKE_MEMORY_PATH>/concepts/`, `<YOKE_MEMORY_PATH>/topics/`, `<YOKE_MEMORY_PATH>/discussions/`, `<YOKE_MEMORY_PATH>/projects/`, `<YOKE_MEMORY_PATH>/fleeting/`):

1. List all `.md` files, **excluding `_template.md` and `_template_node.md`**
   - For actors: include both `<YOKE_MEMORY_PATH>/actors/*.md` (flat) and `<YOKE_MEMORY_PATH>/actors/*/*.md` (folder)
...
2. Do NOT have a corresponding entity file in `<YOKE_MEMORY_PATH>/concepts/` (or any other entity directory)
...
```markdown
## /yoke:compress — Alignment Proposal
...
**Fix:** Add missing backlinks in target entities via /yoke:preserve.
```
...
**Fix:** Create concept entities and add wikilinks in referencing entities via /yoke:preserve.
```
...
**Fix:** Recategorize via /yoke:preserve (create under new type, mark original as promoted/consolidated).
```
...
**Fix:** Create missing entities and establish backlinks via /yoke:preserve.
```
...
**Fix:** Add aliases and wikilinks, or merge duplicate entity files via /yoke:preserve.
```
...
- When invoking `/yoke:preserve`, include in the prompt:
...
- If there are findings in capabilities 2, 3, or 5: compile them into a single fleeting note
  and delegate creation to `/yoke:preserve`:
...
entities:
  - type: fleeting
    ...
    content: |
      ...
      The following alignment issues were detected by `/yoke:compress` running in cron mode.
      Review each proposal and run `/yoke:compress` in interactive mode to execute.
...
- Include in the `/yoke:preserve` invocation:
  "Autonomous mode — do not ask for confirmation, process directly."

---

## Phase 4 — Delegate to /yoke:preserve

### 4.1 Compile structured entity list

Build the entity list in the format accepted by `/yoke:preserve`, grouping all confirmed fixes:
...
`/yoke:preserve` handles adding the wikilink in B's body or frontmatter.
...
- type: <current type>
  ...
  action: update
  content: "> [!info] Content recategorized to [[<new entity name>]]\n> This entity was recategorized by /yoke:compress. See [[<new entity name>]] for the current version."
  relations:
...
- type: <secondary entity's type>
  ...
  action: update
  content: "> [!info] Content consolidated in [[<canonical entity name>]]\n> This entity has been consolidated by /yoke:compress. See [[<canonical entity name>]] for the merged version."
  relations:
    <canonical entity's type plural>: ["<canonical entity name>"]
  source: "compress"
```

### 4.2 Invoke /yoke:preserve

Use the Skill tool to invoke `/yoke:preserve --memory $YOKE_MEMORY_NAME` passing the compiled structured entity list as argument.
The `--memory $YOKE_MEMORY_NAME` flag ensures preserve writes to the same memory.

Include `source: "compress"` for all entities so `/yoke:preserve` records provenance.
...
### Entities processed (via /yoke:preserve)
| Type | Name | Action |
|---|---|---|
...
- Review and run `/yoke:compress` in interactive mode to execute

### Git
- Commit: <hash from /yoke:preserve>
- Push: success / failed (reason)

### Suggestions
- Run `/yoke:status --canonical` for a full vault health report
...
| Situation | Action |
|---|---|
| Empty vault (no entities) | Report "No entities found in the memory." and end |
...
| `/yoke:preserve` fails | Report the error, list what was NOT processed |
...
| Rule | Detail |
|---|---|
| All writes via /yoke:preserve | NEVER use Write or Edit on entity files. Invoke `/yoke:preserve` via the Skill tool. |
...
| User confirmation in interactive | ALWAYS wait for explicit confirmation before delegating to /yoke:preserve in interactive mode. |
...
| Provenance | All entities delegated to /yoke:preserve use `source: "compress"`. |
...
| Vault resolution first | Resolve `YOKE_MEMORY_PATH` before any file operation or git command — never assume CWD is the vault |
| All git commands use `git -C <YOKE_MEMORY_PATH>` | Never assume CWD is the vault |
| All entity paths use `<YOKE_MEMORY_PATH>/` prefix | `<YOKE_MEMORY_PATH>/actors/`, not `actors/` |
| Pass --vault to /preserve | ALWAYS include `--memory $YOKE_MEMORY_NAME` when delegating to `/yoke:preserve` |
