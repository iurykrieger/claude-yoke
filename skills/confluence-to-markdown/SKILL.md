...
description: >
  ...
  and returns Markdown. Used by /yoke:teach and /yoke:teach — not intended for direct user invocation.
...
Internal module — invoked by `/yoke:teach` Phase 1 and `/yoke:teach` Phase 2, not user-invocable.
...
Return to the caller (`/yoke:teach` or `/yoke:teach`):
