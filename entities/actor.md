...
> Source of truth for required fields: `templates/canonical/actor/_template.md`
> Lineage: copied from bedrock 1.2.1 with kebab-case Yoke renames.

## What it is

An **actor** is a system, service, API, or application with its own lifecycle — it has a GitHub repository, an independent deployment process, and is operated by a specific team. Actors are the fundamental infrastructure unit in the canonical memory: each actor represents something that runs in production (or has run, if deprecated).
...
| Field | Type | Description |
|---|---|---|
| `ratified_at` | date | ISO 8601 — when this entry was ratified into canonical memory |
| `model_calibrated_against` | string | Model id this entry was tuned against (e.g., `claude-opus-4-7`) |
| `last_validated` | date | ISO 8601 — last time the entry was re-tested against the current model |
| `traceability` | string | Path to the failure or constraint that justified this entry |
| `impact_level` | string | `low`, `medium`, `high`, or `regulatory` — drives Model C routing |

These five fields are **never deleted on update.** `/yoke:preserve` merges new values; missing values stay missing, present values are updated. Removing any of them produces an unauditable entry and will be rejected.

## Yoke Update Rules

- Body: may be modified and merged — actors hold consolidated, replaceable facts (Recent Activity, Status, etc.).
- Frontmatter: merge new fields, **never** delete existing fields. The five Yoke rippability fields above are mandatory and protected.
- `updated_at` and `updated_by` are always updated.
- New wikilinks may be added to existing arrays; existing wikilinks are never removed.
