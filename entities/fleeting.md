...
> Source of truth for required fields: `templates/canonical/fleeting/_template.md`
> Lineage: copied from bedrock 1.2.1 with kebab-case Yoke renames.

## What it is

A **fleeting note** is a capture of raw information — ideas, emerging concepts, vague mentions, fragments without full context. Fleeting notes are the canonical memory's inbox: they receive content that has not yet reached the threshold of a permanent or bridge note.
...
- Content ingested by `/yoke:teach` contains ideas, mentions, or fragments that do not meet the completeness criteria of any permanent or bridge type
...
- `/yoke:preserve` receives content that does not pass the completeness criteria of any type

## When NOT to create

- The content already has enough data to create a permanent or bridge entity — create directly in the correct type
- The content is irrelevant to the canonical memory (noise, casual conversation, off-topic information)
...
- A new ingestion via `/yoke:teach` brings data that validates or expands the fleeting
...
`/yoke:ask` references the fleeting note in response to a query, signaling that the information is useful:
- The fleeting's content is cited as an answer to a user question
- The fleeting contributes to the understanding of an active subject in the canonical memory
- In this case, `/yoke:ask` signals the promotion opportunity

## Promotion Pipeline

1. **Detection** — `/yoke:preserve`, `/yoke:teach`, or `/yoke:ask` identifies that a promotion criterion has been met
2. **Signaling** — The skill signals with a callout: `> [!info] Suggested promotion: this note can be promoted to <type>`
3. **Promotion** — `/yoke:preserve` is invoked to create the destination entity, migrating the relevant content
...
- Body: may evolve as the note matures, but never delete content from another agent or human.
