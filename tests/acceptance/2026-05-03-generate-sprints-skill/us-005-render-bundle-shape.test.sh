...
#
# Sprint-3 anchors:
#   - sprint task s03-t03 acceptance criterion: "bash exits 0 AND prints
#     `PASS: bundle shape conforms` for every produced sprint file".
#   - functional acceptance criterion id: bundle-shape-conforms-to-template.
#
# Sr Eng's Sprint-3 contract:
#   - synthesize_write_tasks <plan> <tasks-json> <ac-json>
#   - partition_tasks <plan>
#   - render_all_bundles <slug> <plan>
#
# Then-clause (binding):
#   GIVEN the full-flow fixture and a successful pipeline run
#   WHEN every produced .yoke/sprints/<slug>-s<NN>.md file is inspected
#   THEN each MUST carry:
#     (a) exactly the 5 H2 headings in fixed order:
#         `## Sprint objective`, `## Sprint DoD`, `## Tasks`,
#         `## Functional acceptance criteria`, `## Sensors`;
#     (b) every `### Task <ID>` subsection MUST carry the 4 inline labels:
#         `**Story:**`, `**Technical implementation:**`, `**Validation:**`,
#         `**Acceptance criterion:**`;
#     (c) every `**Story:**` line MUST end with a Realizes-clause matching
...
if [[ "$FAIL" -ne 0 ]]; then
  printf '\n--- Result ---\nFAIL: us-005-render-bundle-shape\n' >&2
  exit 1
fi

printf '\n--- Result ---\nPASS: us-005-render-bundle-shape (%d sprint files)\n' "${#PRODUCED[@]}"
exit 0
